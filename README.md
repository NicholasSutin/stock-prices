...
- optimized to be as low-cost as possible (free cloudflare services, free (but limited) API usage)
---
# goal:
- worker calls Polygon.io/Massive for each ticker image once per day, caching the image files in an R2 bucket and updating KV accordingly
- worker calls finnhub for real time quotes, then send the quote data to KV, and that KV data gets read (ISR'd?) every ~5-10 seconds from nextjs
- this avoids clients ever calling polygon or finnhub, ensures relatively real-time updates (daily logo updates, real time ticker prices)
---
...
``.env.local`` has:
```env
LOGO_WORKER_BASE_URL=http://localhost:8787 # development right now? 
